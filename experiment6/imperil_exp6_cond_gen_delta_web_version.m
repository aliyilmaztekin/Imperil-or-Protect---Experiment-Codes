...
for condFile = 1:1
    ...
    % values live in is a 359-position cycle, not a 360-position one -
    % using mod(...,360) here doesn't just risk landing on 0 by chance, a
    % straight 360-cycle can never validly represent this set at all,
    % since it always includes a position with no matching stimulus. Wrap
    % explicitly over 359 so a valid draw is guaranteed by construction.
    ...
    while ~validSetFound && outerAttempts < maxOuterAttempts
        ...
        for col = 1:(nSeriesTotal - 1)
            ...
            while ~success && attempt < maxInnerAttempts
                ...
                % Circular distance checks below still use a 360 modulus.
                % That's fine on its own terms: given two values that are
                % ALREADY guaranteed to be in 1-359 (from wrapHue above),
                % measuring their circular separation as if on a 360-point
                % circle is off by at most a negligible fraction of a
                % degree versus treating it as a true 359-cycle, and does
                % not reintroduce 0 anywhere - only wrapHue's raw value
                % generation could do that, and it no longer can.
...
conditionMatrix(:,5) = reshape(colorsMat((nCoupleColors + 1):end, :), nTrialsTotal, 1);
    ...
    %% Rewrite image names
    frontEnd = [experimentRoot '/TestObjectsTransparent/obj'];
    backEnd1 = '-resized.png';

    for rewrite = 1:nTrials
        curRow = imageMatrix(rewrite,:);        % check orientation
        for item = 1:3
            objID = erase(curRow{item}, {frontEnd, backEnd1});
            if contains(objID, filesep) || contains(objID, '/')
                error('Erase failed on trial %d item %d: %s', rewrite, item, curRow{item});
            end
            curColor = num2str(conditionMatrix(rewrite, 2+item));
            curRow{item} = sprintf('stim_obj%s-resized_deg_%03s.png', objID, curColor);
        end
    ...
    conditionMatrix(:,8:9)   = coords(thetas + 0);
