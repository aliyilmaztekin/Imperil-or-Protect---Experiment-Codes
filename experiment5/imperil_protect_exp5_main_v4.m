...
numAvailableStimuli = numel(stimuliPaths);
...
numMainTargets = totalTrial/seriesLength;
numMainDistractors = totalTrial/seriesLength;
numTrainingTargets = trainingDuration/seriesLength;  
numTrainingDistractors = trainingDuration/seriesLength;

% Randomly select main phase targets
mainTargetIndices = randperm(numAvailableStimuli, numMainTargets);
mainTargetPaths = stimuliPaths(mainTargetIndices);

% Repeat each stimulus 6 times
mainTargets = repelem(mainTargetPaths', 6);

% Randomly select main phase distractors from the remaining pool
remainingIndices = setdiff(1:numAvailableStimuli, mainTargetIndices);
mainDistractorIndices = randsample(remainingIndices, numMainDistractors);
mainDistractorPaths = stimuliPaths(mainDistractorIndices);

% Repeat each stimulus 6 times
mainDistractors = repelem(mainDistractorPaths',6);

% Randomly select training stimuli from the remaining pool
remainingIndices = setdiff(1:numAvailableStimuli, mainDistractorIndices);
trainingTargetIndices = randsample(remainingIndices, numTrainingTargets);
trainingTargetPaths = stimuliPaths(trainingTargetIndices);

% Repeat each stimulus 6 times
trainingTargets = repelem(trainingTargetPaths',6);

% Randomly select training stimuli from the remaining pool
remainingIndices = setdiff(1:numAvailableStimuli, trainingTargetIndices);
trainingDistractorIndices = randsample(remainingIndices, numTrainingDistractors);
trainingDistractorPaths = stimuliPaths(trainingDistractorIndices);

% Repeat each stimulus 6 times
trainingDistractors = repelem(trainingDistractorPaths',6);
...
navy = [0 0 128]; 
orange = [255 164 0];
stimulusColors = [navy; orange];
navyCueText = 'BLUE';
orangeCueText = 'YELLOW';
...
% Series instruction
newSeries = 'NEW'; % Display at the beginning of a new repetition series
ongoingSeries = 'SAME'; % Display if the same series is ongoing
seriesCueRect = CenterRectOnPointd([0 0 stim.size stim.size], centerX, centerY - stim.size);
...
experimentalConditions = NaN(totalTrial,1);
    ...
    outputMatrix(:,16) = conditionMatrix(:,8); % 16th column: Angle of Distractor at Encoding
    outputMatrix(:,17) = conditionMatrix(:,9); % 17th column: Cue relevance (which encoding item is the target and
    % which the distractor).

%% TRAINING BEGINS

greetings =['Welcome to the experiment!\n\n' ...
    'You will now see two objects side by side, tilted at random angles\n' ...
    'One of these objects will be pointed to by a direction cue\n' ...
    'Please keep that object and its orientation in mind. You may ignore the other object\n\n' ...
    'After a short blank screen, you will see two versions of the studied image\n' ...
    'Your task is to indicate which of them is the studied one\n\n' ...
    'Please answer by pressing the up (↑) or down arrow (↓) keys\n' ...
    'You will have 3 seconds to make your selection']; 

enterRepetition = ['The same two images will be repeated for six times in a row\n\n' ...
                   'Right before a new pair of images appears, the screen will show a "NEW" cue\n' ...
                   'As the same pair repeats, a "SAME" cue will appear instead'];
...
if doTraining
    for trainingTrial = 1:trainingDuration
        ...
        elseif trainingTrial == 2
    
            % Greet the participant & describe the task
            Screen('FillRect' , window, [128 128 128], [0 0 screenWidth screenHeight]);
            DrawFormattedText(window, enterRepetition, 'center', 'center', instructionsTextColor);
            Screen('Flip', window);
            waitForSpace();
        ...
        % Display the relevance cue at the start of each repetition series
        if trainingTrial == 1
    
            % Display the relevance cue
            if trainingMatrix(trainingTrial, 9) == 0
                Screen('FillRect', window, [128 128 128], [0 0 screenWidth screenHeight]);
                DrawFormattedText(window, navyCueText, 'center', 'center', instructionsTextColor);
                [~, cueOnset] = Screen('Flip', window);
                WaitSecs(relevanceCueDuration);
            elseif trainingMatrix(trainingTrial, 9) == 1
                Screen('FillRect', window, [128 128 128], [0 0 screenWidth screenHeight]);
                DrawFormattedText(window, orangeCueText, 'center', 'center', instructionsTextColor);
                [~, cueOnset] = Screen('Flip', window);
                WaitSecs(relevanceCueDuration);
            end
        end
        ...
        targetAngle = trainingMatrix(trainingTrial, 6);
        distractorAngle = trainingMatrix(trainingTrial, 8);
        trainingRelevanceCue = trainingMatrix(trainingTrial, 9);

        %% Encoding Screen:
        %% Draw the target image now
        [targetImg, ~, targetAlpha] = imread(trainingTargets{trainingTrial});
    
        % Fix alpha
        if isempty(targetAlpha)
            targetAlpha = 255 * ones(size(targetImg,1), size(targetImg,2), 'uint8');
        elseif isa(targetAlpha, 'double')
            targetAlpha = uint8(targetAlpha * 255);
        end
        
        % Convert to luminance
        targetGray = uint8(mean(targetImg, 3));

        if trainingRelevanceCue == 0
            
            % Paint the picture blue
            targetRGB = stimulusColors(1,:);
            distRGB = stimulusColors(2,:);

            R = uint8(double(targetGray) * targetRGB(1) / 255);
            G = uint8(double(targetGray) * targetRGB(2) / 255);
            B = uint8(double(targetGray) * targetRGB(3) / 255);

        elseif trainingRelevanceCue == 1

            % Paint the picture yellow
            targetRGB = stimulusColors(2,:);
            distRGB = stimulusColors(1,:);
        ...
        targetImgRGBA = cat(3, R, G, B, targetAlpha);

        %% Draw the distractor image now
        [distImg, ~, distAlpha] = imread(trainingDistractors{trainingTrial});
    
        % Fix alpha
        if isempty(distAlpha)
            distAlpha = 255 * ones(size(distImg,1), size(distImg,2), 'uint8');
        elseif isa(distAlpha, 'double')
            distAlpha = uint8(distAlpha * 255);
        end
        
        % Convert to luminance
        distGray = uint8(mean(distImg, 3));

        R = uint8(double(distGray) * distRGB(1) / 255);
        G = uint8(double(distGray) * distRGB(2) / 255);
        B = uint8(double(distGray) * distRGB(3) / 255);

        distImgRGBA = cat(3, R, G, B, distAlpha);
        
        % Make the images into a texture only once per repetition series to
        % avoid redundant computation.
        if mod(trainingTrial, 6) == 1
            targetImageTexture = Screen('MakeTexture', window, targetImgRGBA);
            distImageTexture = Screen('MakeTexture', window, distImgRGBA);
        end

        %% Now show the stimuli against the current context
        Screen('FillRect', window, currentContext, [0 0 screenWidth screenHeight]);
        Screen('DrawLines', window, allCoords, fixationWidth, fixationColor, [centerX centerY], 2);
        Screen('DrawTexture', window, targetImageTexture, [], currentEncodingSite, targetAngle, 0);
        Screen('DrawTexture', window, distImageTexture, [], counterEncodingSite, distractorAngle, 0);
        ...
        if trainingMatrix(trainingTrial, 5) == 0
            ...
            Screen('FillRect', window, currentContext, [0 0 screenWidth screenHeight]);
            Screen('DrawLines', window, allCoords, fixationWidth, fixationColor, [centerX centerY], 2);
            Screen('DrawTexture', window, targetImageTexture, [], currentTestingSite, targetAngle);
            Screen('DrawTexture', window, targetImageTexture, [], counterTestingSite, foilRotationAngle);
       ...
       elseif trainingMatrix(trainingTrial, 5) == 1
        ...
        while GetSecs < testingLimit && ~keyPressed
            ...
            if keyIsDown
                if keyCode(KbName('UpArrow'))
                    % If "up" is keyed in, left half is chosen
                    selectedSide = 0;
                ...
                elseif keyCode(KbName('DownArrow'))
                    % If "down" is keyed in, right half is chosen
                    selectedSide = 1;
                    keyPressed = true;
                    KbReleaseWait;
        ...
        if mod(trainingTrial,6) == 0
            % Close the study image texture at the end of repetition series to
            % save memory.
            Screen('Close', targetImageTexture);
            Screen('Close', distImageTexture);
        end
...
for block = 1:totalBlock
    ...
    for trial = 1:trialPerBlock
       ...
       %% At the beginning of each block, show the relevance cue
       % Only the encoding item with the color indicated here is relevant

       if trial == 1
           if conditionMatrix(liveTrial, 9) == 0
                % Blue items are relevant
    
                targetColor = stimulusColors(1,:);
                distColor = stimulusColors(2,:);
           ...
           elseif conditionMatrix(liveTrial, 9) == 1
                % Yellow items are relevant

                targetColor = stimulusColors(2,:);
                distColor = stimulusColors(1,:);
       ...
       if eyeTrack
        ...
        % Overlay the series cue on the background
        if conditionMatrix(liveTrial, 2) == 1
            DrawFormattedText(window, newSeries, 'center', (centerY - stim.size), instructionsTextColor);
        else 
            DrawFormattedText(window, ongoingSeries, 'center', (centerY - stim.size), instructionsTextColor);
        end
        ...
        targetAngle = conditionMatrix(liveTrial, 6);
        distAngle = conditionMatrix(liveTrial, 8);

        % Draw the target image
        [targetImg, ~, targetAlpha] = imread(mainTargets{liveTrial});
        ...
        % Colorize
        R = uint8(double(targetGray) * targetColor(1) / 255);
        G = uint8(double(targetGray) * targetColor(2) / 255);
        B = uint8(double(targetGray) * targetColor(3) / 255);
        
        targetImgRGBA = cat(3, R, G, B, targetAlpha);

        % Draw the distractor image
        [distImg, ~, distAlpha] = imread(mainDistractors{liveTrial});
        ...
        % Colorize
        R = uint8(double(distGray) * distColor(1) / 255);
        G = uint8(double(distGray) * distColor(2) / 255);
        B = uint8(double(distGray) * distColor(3) / 255);
        
        distImgRGBA = cat(3, R, G, B, distAlpha);
        
        % Make into texture only once in a repetition series
        if mod(trial,6) == 1
            targetImageTexture = Screen('MakeTexture', window, targetImgRGBA);
            distImageTexture = Screen('MakeTexture', window, distImgRGBA);
        end
        ...
        Screen('DrawLines', window, allCoords, fixationWidth, fixationColor, [centerX centerY], 2);
        Screen('DrawTexture', window, targetImageTexture, [], currentEncodingSite, targetAngle, 0);
        Screen('DrawTexture', window, distImageTexture, [], counterEncodingSite, distAngle, 0);
