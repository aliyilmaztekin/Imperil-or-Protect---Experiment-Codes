...
stimuliDIR = 'C:\Users\gamalab1\Desktop\Ali Yılmaztekin - Imperil 5\testObjectsTransparent';
...
leftHalfKey = KbName('LeftArrow');
rightHalfKey = KbName('RightArrow');
...
experimentalConditions = NaN(totalTrial,1);
    ...
    outputMatrix(:,6) = conditionMatrix(:,3);  % 7th column: context change 
    % 7th column: Current Context RGB (only the first number) 
    outputMatrix(:,8) = conditionMatrix(:,4); % 9th column: Encoding site 
    outputMatrix(:,9) = conditionMatrix(:,5); % 10th column: Testing site
    outputMatrix(:,10) = conditionMatrix(:,6); % 11th column: Target angle
    outputMatrix(:,11) = conditionMatrix(:,7); % 12th column: Foil angle
...
for block = 1:totalBlock
    ...
    for trial = 1:trialPerBlock
       ...
       if eyeTrack
        ...
        % If the response was correct
        elseif (keyPressed && (testingSite == 0 && selectedSide == 0)) || (keyPressed && ...
               (testingSite == 1 && selectedSide == 1))
        ...
        % If the response was incorrect
        elseif (keyPressed && (testingSite == 0 && selectedSide == 1)) || (keyPressed && ...
