# Imperil or Protect - Experiment 3 - ANOVA/LMM/Gamma GLMM analysis
# Coded by A.Y.
...
combinedData_firstpass <- combinedData %>%
  dplyr::mutate(raw_angle = ((angle + 180) %% 360) - 180) %>%
...
message("Excluded subjects (mean abs circular error > 60 deg):")
print(bad_subjects)
...
writeMat("/Users/ali/Desktop/Imperil-or-Protect---Experiment-Codes/experiment3/mixture_modelling/combinedData_thirdpass3.mat",
...
writeMat("/Users/ali/Desktop/Imperil-or-Protect---Experiment-Codes/experiment3/excluded_trials_exp3.mat", miss_trials = named)
...
summary(glmm_mod_full)
car::Anova(glmm_mod_full, type = 2)       
rep_emm <- emmeans(glmm_mod_full, ~  interference | repetition, type = "response")  
...
out_name <- if (dependent_variable == "angle") "exp3_error_plot.png" else "exp3_time_plot.png"

ggsave(
  filename = file.path(save_dest, out_name),
  plot   = exp3_ws_hybrid_plot_out,
