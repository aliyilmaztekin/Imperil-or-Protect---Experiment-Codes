...
# -------------------------------------------------------------------------
# SWAP COLUMN NAME
# The swap-model output adds a swap/misbinding-rate column. Set this to
# whatever it is called in your CSVs. Common names: "B" (MemToolbox /
# Bays 2009), "swap", "swap_rate", "beta". Everything downstream reads the
# column through this variable, so this is the ONLY place you change it.
# Both CSVs (rep1_rep5 and all_reps) are assumed to contain this column.
# -------------------------------------------------------------------------
swap_col <- "swap"
...
# (this is your existing file; it already has named columns incl. g, SD, swap)
data_csv <- sprintf(
  "/Users/ali/Desktop/Imperil-or-Protect---Experiment-Codes/experiment3/swap_model/param_out/exp_3_swap_full_model.csv",
...
data_csv_allreps <- sprintf(
  "/Users/ali/Desktop/Imperil-or-Protect---Experiment-Codes/experiment3/swap_model/param_out/exp_3_swap_reps_only.csv", 
  exp_handle, exp_handle
)
...
# --- swap frame (treated like g: a bounded rate in [0,1], logit for models)
...
m_g_ctx <- lmerTest::lmer(
  logit_g ~ repetition * context * interference +
    (1 | subject),
  data = df_ctx_g
)
cat("\n===== g : omnibus (Type II F) =====\n")
...
## ---- swap : logit scale (mirrors g exactly) -----------------------------
m_swap_ctx <- lmerTest::lmer(
  logit_swap ~ repetition * context * interference + (1 | subject),
  data = df_ctx_swap
)
cat("\n===== swap : omnibus (Type II F) =====\n")
print(car::Anova(m_swap_ctx, type = 2, test.statistic = "F"))

emms_swap <- emmeans(ref_grid(m_swap_ctx, tran = "logit"),
                     ~  interference | repetition + context , type = "response")
cat("\n===== swap : interference | repetition (pairwise, odds-ratio scale) =====\n")
print(contrast(emms_swap, method = "pairwise", infer = TRUE))

m_swap_rep <- lmerTest::lmer(logit_swap ~ repetition + (1 | subject), data = df_rep)
...
make_hybrid_plot <- function(dv, y_label, step_size, y_round,
  ...
  link <- match.arg(link)
  lf <- switch(link,
               identity = list(fwd = function(x) x,
                               inv = function(x) x),
               logit    = list(fwd = function(x) qlogis(pmin(pmax(x, 1e-4), 1 - 1e-4)),
                               inv = plogis),
               log      = list(fwd = function(x) log(pmax(x, 1e-8)),
                               inv = exp)
  )
  
  # everything downstream works on the link scale; back-transform only for plotting
  d_ctx <- d_ctx %>% dplyr::mutate(tval = lf$fwd(.data[[dv]]))
  d_rep <- d_rep %>% dplyr::mutate(tval = lf$fwd(.data[[dv]]))
  
  # within-subject summary on tval, returns back-transformed mean_error / lo / hi
  sew_bt <- function(data, withinvars) {
    out <- Rmisc::summarySEwithin(
      data = data, measurevar = "tval",
      withinvars = withinvars, idvar = "subject", conf.interval = 0.95
    )
    out$mean_error <- lf$inv(out$tval)
    out$lo         <- lf$inv(out$tval - out$ci)
    out$hi         <- lf$inv(out$tval + out$ci)
    out
  }
  ...
  ws_df_rep_all <- d_rep %>%
    ...
    dplyr::summarise(tval = mean(tval, na.rm = TRUE), .groups = "drop")
  
  ws_rep_all <- sew_bt(ws_df_rep_all, withinvars = "repetition") %>%
  ...
  ws_rep_int <- sew_bt(ws_df_rep_int, withinvars = c("repetition", "interference")) %>%
    dplyr::mutate(
  ...
  ws_summary_rep <- dplyr::bind_rows(
    ws_rep_int %>% dplyr::select(repetition, interference, mean_error, lo, hi),
    ws_rep_mid %>% dplyr::select(repetition, interference, mean_error, lo, hi)
  ) %>%
    dplyr::mutate(
      repetition   = factor(repetition, levels = x_levels),
      interference = factor(interference, levels = int_levels)
  ...
  ws_summary_ctx_int <- sew_bt(ws_df_ctx_int,
  ...
  range_df <- if (!is.null(y_range)) {
    data.frame(mean_error = y_range, rep_x = 1)
  } else NULL
  
  ggplot() +
    geom_ribbon(
      data = ws_summary_rep,
      aes(x = rep_x, ymin = lo, ymax = hi, group = interference),
    ...
    geom_errorbar(
      ...
      aes(x = repetition, ymin = lo, ymax = hi),
    ...
    geom_errorbar(
      data = ws_summary_ctx_int,
      aes(x = repetition, ymin = lo, ymax = hi, color = context),
...
p_SD <- make_hybrid_plot(
  ...
  y_range = c(16, 25.5),
  link = "identity"        # SD is unbounded-positive; leave raw (or "log")
)
...
p_g <- make_hybrid_plot(
  ...
  y_percent = TRUE,
  y_range = c(0, 0.19),
  link = "logit"           # bounded rate -> asymmetric CIs
)

p_swap <- make_hybrid_plot(
  dv = swap_col, y_label = "Swap Rate",
  step_size = if (exp_handle %in% c(1:2)) 0.02 else 0.05,
  y_round = 2.5,
  d_ctx = df_ctx_swap, d_rep = df_rep,
  y_percent = TRUE,
  y_range = NULL,
  link = "logit"
)
...
ggsave(
  filename = file.path(save_dest, sprintf("exp%d_mixture_SD_hybrid.png", exp_handle)),
  plot = p_SD, width = 12, height = 7, units = "in", dpi = 300
)

ggsave(
  filename = file.path(save_dest, sprintf("exp%d_mixture_g_hybrid.png", exp_handle)),
  plot = p_g, width = 12, height = 7, units = "in", dpi = 300
)

ggsave(
  filename = file.path(save_dest, sprintf("exp%d_mixture_swap_hybrid.png", exp_handle)),
  plot = p_swap, width = 12, height = 7, units = "in", dpi = 300
)
