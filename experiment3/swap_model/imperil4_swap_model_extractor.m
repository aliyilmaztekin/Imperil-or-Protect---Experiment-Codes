...
elseif exp == 6
    % Data file DIR template
    filename = sprintf('/Users/ali/Desktop/Imperil-or-Protect---Experiment-Codes/experiment%d/swap_model/combinedData_thirdpass%d.mat', exp, exp);
    load(filename)
end
...
if mode == 1
    ...
    % Exp 6 has no interference factor — one dummy level
    if exp == 6 || exp == 4
        interfLevels = 0;
        if exp == 6
            probeItems = 1:2; % 1 -> Old1 & Old2; 2 -> Novel
        elseif exp == 4
            probeItems = 1:2; % 1 -> Test 1 (novel item); 2 -> Test 2 (repeated item)
        end
    else
        interfLevels = 0:1;
        probeItems = 1;
    ...
    for probe = probeItems
    for rep = [1 5]
    for context = 0:1
    for interference = interfLevels
    
        % If the probe idx is 1, pool repeated items together
        if exp == 6 && probe == 1

            mask = ismember(thirdpass(:, testedItem), [1 2]) ...
                 & thirdpass(:, subCol)     == subID ...
                 & thirdpass(:, repCol)     == rep ...
                 & thirdpass(:, contextCol) == context;
          
        % If probe is 2, search for novel item trials
        elseif exp == 6 && probe == 2
            mask = thirdpass(:, testedItem) == 3 ...
                & thirdpass(:, subCol)     == subID ...
                & thirdpass(:, repCol)     == rep ...
                & thirdpass(:, contextCol) == context;
        end
        ...
        if exp == 3
            mask   = mask & thirdpass(:, interfCol) == interference;
            ntCols = nonTargetErrCol;      % 1 non-target
        else
            ntCols = nonTargetErrCols;     % 2 non-targets
        end
    
        angError       = thirdpass(mask, errCol);
        nonTargetError = thirdpass(mask, ntCols);
    
        % Any NaN in .distractors makes the likelihood NaN
        ok             = ~isnan(angError) & all(~isnan(nonTargetError), 2);
        angError       = angError(ok);
        nonTargetError = nonTargetError(ok, :);
        ...
        data.distractors = nonTargetError';   % (nItems-1) x nTrials
    try
    ...
    %   sd - precision
                        ...
                        subResults.probe(localRow,1)      = probe;
    ...
    % If the model fit fails, inform and move on
                            ...
                            fprintf('skip subj %d rep %d ctx %d intf %d: %s\n', ...
                                subID, probe, rep, context, interference, ME.identifier);
    continue
    end
    end
    end
    ...
    results.probe        = categorical(results.probe);
